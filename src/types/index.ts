 ...
 * DB table: communities (legacy name)
 */
 ...
 * DB table: community_admins (legacy name)
 */
export interface OrganisationMember {
  id: string;
  user_id: string;
  organisation_id: string; // DB column: community_id
  assigned_at: string;
}

/**
 * Introduction group within an organisation.
 * DB table: programmes (legacy name)
 */
export interface Group {
  ...
  name: string;
  description: string | null;
  group_type: "ongoing" | "cohort_blitz" | "event"; // DB: programme_type with legacy values
  is_active: boolean;
  created_at: string;
  updated_at: string;
}
...
export interface Introduction {
  ...
  organisation_id: string; // DB column: community_id (if exists)
